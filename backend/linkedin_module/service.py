...
async def _build_person_lead(post: dict, kept_meta: dict) -> Lead | None:
    ...
    return _make_lead(
        ...
        post_content=post_content, query=query,
    )


async def _build_one_company_person(
    person: dict, signal_strength: str, post_content: str, query: str
) -> Lead | None:
...
async def _build_company_leads(post: dict, kept_meta: dict, icp: dict) -> list[Lead]:
    author = post.get("author") or {}
    ...
    logger.info("[linkedin] company post: %s | signal=%s", company_name, signal_strength)
    ...
    results = await asyncio.gather(
        *[_build_one_company_person(p, signal_strength, post_content, query) for p in people],
        return_exceptions=True,
    )
    leads = [r for r in results if isinstance(r, Lead)]
    logger.info("[linkedin] ✓ %d leads from %s", len(leads), company_name)
    return leads


async def _process_post(post: dict, kept_meta: dict, icp: dict) -> list[Lead]:
    author = post.get("author") or {}
    linkedin_url: str = author.get("linkedinUrl") or author.get("linkedin_url") or ""
    if "/company/" in linkedin_url:
        return await _build_company_leads(post, kept_meta, icp)
    lead = await _build_person_lead(post, kept_meta)
    return [lead] if lead else []


async def run(icp: dict, product: str, session_id: str | None = None) -> list[Lead]:
    logger.info("[linkedin] ═══ start ═══")
    ...
    posts = await search_linkedin_posts(queries)
    if not posts:
        return []
    ...
    logger.info("[linkedin] processing %d kept posts", len(relevant))
    tasks = [
        _process_post(p, kept_index[p.get("id") or p.get("entityId", "")], icp)
        for p in relevant
    ]
